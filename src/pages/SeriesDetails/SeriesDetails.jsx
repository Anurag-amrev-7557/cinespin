...
const MovieDetails = () => {
    ...
    useEffect(() => {
        const fetchMovieDetails = async () => {
            ...
            try {
                    ...
                    try {
                        ...
                        const collectionWithCredits = await Promise.all(
                            (collectionData.parts || []).filter(m => m.id !== data.id).map(async (part) => {
                                const partRes = await fetch(`${TMDB_BASE_URL}/tv/${part.id}?api_key=${TMDB_API_KEY}&append_to_response=credits`);
                ...
                try {
                    const similarRes = await fetch(`${TMDB_BASE_URL}/tv/${id}/similar?api_key=${TMDB_API_KEY}`);
                    const similarData = await similarRes.json();
                    const similarWithCredits = await Promise.all(
                        (similarData.results || []).slice(0, 10).map(async (sim) => {
                            const detailsRes = await fetch(`${TMDB_BASE_URL}/tv/${sim.id}?api_key=${TMDB_API_KEY}&append_to_response=credits,videos`);
    ...
    return (
        <motion.div className="movie-details-container" variants={animations.container} initial="hidden" animate="visible" exit="exit">
            <Helmet>
                <title>{movie.name} - Series Details</title>
            ...
            <motion.div className="movie-content" variants={animations.fadeUp}>
                <motion.div className="movie-info" variants={animations.fadeUp}>
                    ...
                    <div className="movie-meta">
                        ...
                        <motion.span variants={animations.fadeUp}><FaCalendar /> {(movie.first_air_date || movie.release_date || "").split("-")[0]}</motion.span>
                        ...
                        {movie.number_of_seasons !== undefined && (
                            <motion.span variants={animations.fadeUp} style={{display: "flex", alignItems: "center", fontSize: "0.9vw"}}>
                    ...
                    <motion.div className="movie-genres" variants={animations.fadeUp}>
                        {movie.genres.map(genre => (
                            <motion.div
                                ...
                                role="button"
                                tabIndex={0}
                                onClick={() => navigate(`/series?genre=${genre.id}&page=${currentPage}`)}
                                onKeyDown={(e) => {
                                    if (e.key === 'Enter') {
                                        navigate(`/series?genre=${genre.id}&page=${currentPage}`);
                                    }
                                }}
                        ...
                        <motion.div className="similar-movies" variants={animations.fadeUp}>
                            ...
                            <div className="similar-list">
                                {movie.similar.results
                                    ...
                                    .map(sim => (
                                        <motion.a key={`${sim.id}-${sim.first_air_date || ""}-${Math.random()}`} className="similar-movie" onClick={() => navigate(`/movie/${sim.id}`)} variants={animations.fadeUp} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
