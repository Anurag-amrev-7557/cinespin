...
const Movies = () => {
    ...
    const handleGenreClick = (genreId) => {
        if (genreId !== selectedGenre) {
            ...
            navigate(`/series?genre=${genreId}&page=${currentPage}`);
    ...
    const fetchMovieDetails = async (movieId) => {
        try {
            const response = await fetch(
              `https://api.themoviedb.org/3/tv/${movieId}?api_key=${TMDB_API_KEY}`
            );
    ...
    const fetchMovies = async (genreId = 0, page = 1) => {
        ...
        try {
            ...
            const url = `https://api.themoviedb.org/3/discover/tv?api_key=${TMDB_API_KEY}${genreParam}&page=${safePage}${regionParam}`;
            ...
            setPageRangeStart(Math.floor((safePage - 1) / 5) * 5 + 1);
            navigate(`/series?genre=${safeGenreId}&page=${safePage}`, { replace: true });
                    ...
                    >
                        {sortedMovies.map((movie) => (
                            ...
                            >
                                ...
                                {(movie.name || movie.title)?.split(" ").slice(0, 3).join(" ")}
                                </span>
