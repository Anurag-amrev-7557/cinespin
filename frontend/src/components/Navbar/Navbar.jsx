...
import "./Navbar.css"; // Ensure styling is handled here
...
const Navbar = () => {
    ...
    const [selectedRegion, setSelectedRegion] = useState(() => {
        // Retrieve region from localStorage, default to "Global" if not found
    ...
    const handleRegionChange = (region) => {
        ...
        localStorage.setItem('region', region); // Save region to localStorage
        setIsPopupOpen(false);
    
        // Trigger any necessary content update without reloading or navigating
    ...
    const handleScroll = useCallback(() => {
        if (window.scrollY > 100) { // Trigger animation when scrolled 100px down
            setIsVisible(true);
        } else {
            setIsVisible(false);
        }
    ...
    useEffect(() => {
        ...
        document.querySelector(".navbar-search").classList.toggle('dark-mode', isDarkMode);
        setTimeout(() => {
    ...
    return (
        <nav className={`navbar ${isVisible ? "fade-in" : "fade-out"}`} ref={navbarRef}>
            ...
            <label className="popup">
                <input type="checkbox" checked={isPopupOpen} onChange={() => setIsPopupOpen(!isPopupOpen)} />
                ...
                <nav className="popup-window" ref={popupRef}>
                    ...
                    <li>
                        <button onClick={() => handleRegionChange("India")}>
                        <span>India</span>
                        </button>
                    </li>
                    </ul>
